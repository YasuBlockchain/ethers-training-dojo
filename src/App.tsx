...
import {CHAIN_POLYGON, CHAIN_GOERLI, ChainList} from "./constants";
                                        ...
                                        {erc20.events!.transferFrom!.length > 0 ?
                                            <>
                                                <div className={"row pt-3"}>
                                                    ...
                                                    <h6 className={"col-2"}>Block (position)</h6>
                                                    <h6 className={"col-4"}>To</h6>
                                                    <h6 className={"col-2"}>Value in ETH</h6>
                                                </div>
                                                <hr/>
                                                {
                                                    erc20.events?.transferFrom?.map((item, key) => (
                                                        <div className={"row mb-2"} key={key}>
                                                            <div className={"col-4"}>{item.transactionHash}</div>
                                                            <div className={"col-2"}>{item.blockNumber} ({item.transactionIndex}th)</div>
                                                            <div className={"col-4"}>{item.args?.to}</div>
                                                            <div className={"col-2"}>{ethers.utils.formatEther(item.args?.value)}</div>
                                                        </div>
                                                    ))
                                                }
                                    ...
                                    <div className={"pt-3"}>
                                        ...
                                        {erc20.events!.transferTo!.length > 0 ?
                                            <>
                                                ...
                                                <hr/>
                                                {
                                                    erc20.events?.transferTo?.map((item, key) => (
                                                        <div className={"row mb-2"} key={key}>
                                                            <div className={"col-4"}>{item.transactionHash}</div>
                                                            <div className={"col-2"}>{item.blockNumber} ({item.transactionIndex}th)</div>
                                                            <div className={"col-4"}>{item.args?.to}</div>
